...
type AppConfig struct {
	...
	// DefaultModel is the "<provider>:<model>" spec used when no per-run
	// override is supplied. CLI --model and POST /v1/runs `model` both
	// take precedence; if all three are empty, provider.DefaultModelSpec
	// applies. Empty/absent is fine — most apps don't care which model
	// drives them.
